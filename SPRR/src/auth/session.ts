...
/**
 * 从 cookie 字符串构造 SessionData（无需 storageState 文件）
 *
 * 适用场景：用户直接传入从浏览器/APP 抓包的 cookie 字符串。
 *
 * @param cookieStr cookie 字符串（格式: "k1=v1; k2=v2; ..."）
 */
export function sessionFromCookieString(cookieStr: string): SessionData {
  const cookiesMap: Record<string, string> = {};
  for (const part of cookieStr.split(';')) {
    const s = part.trim();
    if (!s) continue;
    const idx = s.indexOf('=');
    if (idx <= 0) continue; // 跳过空名或无等号条目
    const name = s.slice(0, idx);
    const value = s.slice(idx + 1);
    cookiesMap[name] = value;
  }
  if (Object.keys(cookiesMap).length === 0) {
    throw new Error('cookie 字符串解析失败：未找到任何有效 cookie');
  }
  ...
  log.info(
    `从 cookie 字符串加载 ${Object.keys(cookiesMap).length} 个 cookie（uid_tt=${cookiesMap['uid_tt'] || '?'}, sessionid=${cookiesMap['sessionid'] ? '有' : '无'}）`,
  );
  return session;
}
