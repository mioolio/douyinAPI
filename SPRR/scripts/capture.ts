...
const TARGET_URL = getArg('url', 'https://www.douyin.com/chat?isPopup=1');
const STORAGE_STATE = getArg(
  'state',
  path.resolve(__dirname, '..', '..', 'ccc', 'data', 'storageState.json'),
);
...
const UPLOAD_DIR = path.join(CAPTURE_DIR, 'upload'); // ★ 图片上传请求（重点捕获）
...
/** ★ 判断是否为图片上传请求（不限域名）
 *  上传请求的特征：
 *  1. URL 含 upload/apply_upload/commit_upload/tos/mossrv 等关键词
 *  2. content-type 是 multipart/form-data（图片直传 TOS 的典型特征）
 *  3. POST/PUT 方法 + content-type 含 octet-stream（二进制直传）
 */
const UPLOAD_KEYWORDS = [
  'upload', 'apply_upload', 'commit_upload', 'init_upload',
  'tos', 'mossrv', 'tosvolc', 'uploadv2', 'zijieapi', 'volc',
  'image/upload', 'file/upload', 'media/upload',
];

function isUploadRequest(url: string, method: string, contentType: string): boolean {
  const urlLower = url.toLowerCase();
  const ctLower = contentType.toLowerCase();
  // URL 关键词匹配（不限域名）
  if (UPLOAD_KEYWORDS.some((kw) => urlLower.includes(kw))) return true;
  // multipart/form-data（图片上传典型 content-type）
  if (ctLower.includes('multipart/form-data')) return true;
  // POST/PUT + octet-stream（二进制直传）
  if ((method === 'POST' || method === 'PUT') && ctLower.includes('octet-stream')) return true;
  return false;
}
...
async function ensureDirs() {
  ...
  await fs.mkdir(UPLOAD_DIR, { recursive: true }); // ★ 上传请求单独保存
...
const ENDPOINT_CATEGORIES: Array<{ pattern: RegExp; category: string }> = [
  ...
  // ★ 图片上传相关 endpoint
  { pattern: /\/aweme\/v1\/web\/im\/.*upload/, category: 'im_upload' },
  { pattern: /apply_upload/, category: 'apply_upload' },
  { pattern: /commit_upload/, category: 'commit_upload' },
  { pattern: /init_upload/, category: 'init_upload' },
  { pattern: /\/upload\//, category: 'upload' },
  { pattern: /uploadv2/, category: 'upload_tos' },
  { pattern: /mossrv|tosvolc/, category: 'upload_tos' },
];
...
async function saveRequestPair(req: Request, res: Response | null) {
  ...
  // 请求头（先拿，isUpload 判断需要 content-type）
  const reqHeaders = await req.allHeaders();
  const isApi = isApiRequest(url);
  // ★ 上传请求强制视为 API（不限域名，multipart/二进制直传也捕获）
  const isUpload = isUploadRequest(url, method, reqHeaders['content-type'] || '');
  ...
  if (isApi || isUpload) {
    ...
    // ★ 上传请求单独保存到 UPLOAD_DIR（重点分析对象）
    if (isUpload) {
      const uploadFile = path.join(UPLOAD_DIR, baseName + '.json');
      await fs.writeFile(uploadFile, JSON.stringify(record, null, 2));
      const ctTag = reqContentType.includes('multipart') ? '[multipart]'
        : reqContentType.includes('octet-stream') ? '[octet-stream]'
        : reqContentType.includes('protobuf') ? '[protobuf]'
        : '';
      const bodySize = reqBodyBuffer ? reqBodyBuffer.length : (reqBodyText ? reqBodyText.length : 0);
      log('upload', `#${reqCounter} ${method} ${resStatus} ${ctTag} body=${bodySize}B ${url.slice(0, 150)}`);
    }
