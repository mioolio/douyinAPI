...
const OUTPUT_FILE = path.resolve(__dirname, '..', 'data', 'builder-v3.json');
const SCRIPT_FILE = path.resolve(__dirname, 'probe-builder-v3.js');
...
async function main() {
  ...
  // 先点击会话列表第一项，确保 conversationMap 有数据
  console.log('点击会话列表第一项...');
  try {
    const firstItem = page.locator('[data-e2e="conversation-item"]').first();
