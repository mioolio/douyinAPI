...
export async function editProfile(
  ...
  log.info(`editProfile: POST /aweme/v1/web/commit/user/ body=${bodyStr}`);
  const res = await fetch(url.toString(), { method: 'POST', headers, body: bodyStr });
  if (!res.ok) {
  ...
  if (j.status_code !== 0) {
    ...
    return {
      success: false,
      message: `commit/user status_code=${j.status_code}${toast ? ` (${toast})` : ''}`,
    };
  }
