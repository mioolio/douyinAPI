...
export interface ContactItem {
  ...
  isAiBot?: boolean;
  /** 头像 URL（来自 /aweme/v1/web/im/user/info/ 的 avatar_small，168x168） */
  avatarUrl?: string;
 ...
 *   - recall      撤回消息（msgType=40001 服务器撤回通知，或原消息被替换为 "Recall Content Hided"）
 *   - unknown     未知类型
 */
...
export interface MessageItem {
  ...
  videoAuthor?: string;
  /** 视频分享：跳转 URL（https://www.douyin.com/video/<aweme_id>）；其他：undefined */
  videoUrl?: string;
  /** 表情贴纸：图片 URL；其他：undefined */
  stickerUrl?: string;
  /** 是否为加密图片（msgType=91，阅后即焚或永久加密），需要调用 getReadOnceImage 解密 */
  isEncryptedImage?: boolean;
  /** 加密图片是否已被解密查看过（仅 isEncryptedImage 时有意义，由服务层查数据库填充） */
  decrypted?: boolean;
  /** 是否为永久加密图片（is_change_view=false），与阅后即焚区分用于前端标签显示 */
  isPermanent?: boolean;
  /** 是否为撤回的图片消息（原消息被标记为"Recall Content Hided"但保留图片字段） */
  isRecalled?: boolean;
  /**
   * 图片 skey（AES-256-GCM 解密密钥，32 字节 hex 字符串）
   * 普通图片（msgType=27）：来自 content.resource_url.skey，需对下载内容做 AES 解密
   * 加密图片（msgType=91）：来自 read_once/detail 接口
   */
  imageSkey?: string;
  /**
   * 图片 oid（明文 URI，如 tos-cn-o-00061/uploadv2_xxx）
   * 自己发送的图片消息中只有 oid 没有 URL，需调 batch_build_image 换签名 URL
   */
  imageOid?: string;
  contentJson?: string;
  timestamp?: number;
...
function parseUserConvListResponse(
  ...
  for (const f of findFields(subFields, 1)) {
    ...
    if (badgeSeq !== undefined || readBadgeSeq !== undefined) {
      log.info(
        `parseConv: cid=${cid.slice(-12)} badgeSeq=${badgeSeq} readBadgeSeq=${readBadgeSeq} unread=${unreadCount}`,
      );
    }
  }
  return items;
...
function parseMessageBody(
  ...
  try {
    ...
    let imageOid: string | undefined;

    if (contentObj && typeof contentObj === 'object') {
      // 撤回消息（msgType=40001，content 通常为空 {}）
      // 这是服务器推送的撤回通知，非用户操作产生的原消息
      if (msgType === 40001) {
        log.info(`[撤回诊断] msgType=40001 撤回通知，content=${JSON.stringify(contentObj)}, sender=${sender}, serverId=${serverIdBig}`);
        return null;
      }
      ...
      // 视频分享（aweType=800, msgType=8）
        ...
        // 提取 aweme_id 构造跳转 URL（字段名可能是 aweme_id 或 item_id）
        const awemeId = contentObj.aweme_id || contentObj.item_id;
        if (awemeId) {
      ...
      // 图片消息（aweType=702/2702, msgType=2/27/91）
        ...
        category = 'image';
        // 提取图片 URL + skey：优先 resource_url.large_url_list，其次 resource_url.url_list
        ...
        if (resUrl && typeof resUrl === 'object') {
          ...
          // 提取 oid（自己发送的图片无 URL，只有 oid，需调 batch_build_image 换签名 URL）
          if (typeof resUrl.oid === 'string') {
            imageOid = resUrl.oid;
          }
        }
        if (!text) {
          // 无直接 URL 的图片，分三种情况：
          // a) msgType=91 且无 skey → 阅后即焚加密图片（skey 不在消息中，需调 read_once/detail 获取）
          // b) 有 skey → 普通加密图片（自己/对方发送，skey 在消息中，用 batch_build_image 换 URL）
          // c) 其他 → 真正未知（无 URL、无 skey、非 msgType=91）
          const md5 = typeof contentObj.md5 === 'string' ? contentObj.md5 : '';
          const fileMatch = md5.match(/([A-Za-z0-9_]+\.(?:jpg|jpeg|png|webp|gif|bmp))/i);
          if (msgType === 91 && !imageSkey) {
            // 阅后即焚：skey 不在消息中，需调 read_once/detail 获取
            isEncryptedImage = true;
            const isChangeView = contentObj.is_change_view;
            if (isChangeView === false || isChangeView === 0) {
              // 永久加密图片（is_change_view=false）：消息保留
              isPermanent = true;
              text = fileMatch ? `[永久图片:${fileMatch[1]}]` : '[永久图片]';
            } else {
              // 阅后即焚（is_change_view=true 或未设置）：看完后消息销毁
              text = fileMatch ? `[加密图片:${fileMatch[1]}]` : '[加密图片]';
            }
          } else if (imageSkey) {
    ...
    // 服务端同时会清除图片字段，但数据库中可能保留着原始信息
    // 将此类消息标记为 isRecalled=true，后续从数据库恢复图片信息
    ...
    if (isFromRobot) {
      ...
      // msgType=40001 是服务器推送的撤回通知，显示"服务器"
      senderLabel = '服务器';
    } else {
