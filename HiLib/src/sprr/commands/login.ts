...
export async function loginAccount(
  ...
  // 拦截抖音风控/反爬检测域名（if-rc1.yhgfb-cn-static.com 等）
  // 这些域名会通过 window.open 反复弹出标签页
  await context.route('**/*', (route) => {
    const url = route.request().url();
    if (
      url.includes('yhgfb-cn-static.com') ||
      url.includes('if-mcp') ||
      url.includes('rc1.yhgfb')
    ) {
      route.abort();
    } else {
      route.continue();
  ...
  // 新标签页一打开就立即关闭（防止风控脚本 window.open 弹窗）
  context.on('page', async (newPage) => {
    try {
      const url = newPage.url();
      log.debug(`检测到新标签页，立即关闭: ${url}`);
      await newPage.close();
    } catch {
      // ignore
    }
