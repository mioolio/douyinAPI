 ...
 *   4. DEFAULT_STORAGE_STATE   兜底（data/storageState.json）
 */
...
export function accountFile(name: string): string {
...
export async function listAccounts(): Promise<
  Array<{ name: string; file: string; savedAt: number; size: number; uid?: string; hasSessionid: boolean }>
> {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      const j = JSON.parse(raw) as { cookies?: Array<{ name: string; value: string }> };
...
/** 设置当前账号指针 */
export async function setCurrentAccount(name: string): Promise<void> {
  validateAccountName(name);
...
export async function resolveStorageState(
  ...
  return { path: DEFAULT_STORAGE_STATE, source: '默认兜底（data/storageState.json）' };
}
