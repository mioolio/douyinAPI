...
export const DEFAULT_STORAGE_STATE = path.resolve(PROJECT_ROOT, '..', 'ccc', 'data', 'storageState.json');
/** 聊天页 URL */
...
export async function launchBrowser(opts = {}) {
    const { storageState = DEFAULT_STORAGE_STATE, headless = false, sdkTimeoutMs = 60_000, } = opts;
 ...
 */
export async function waitForSdk(page, timeoutMs = 60_000) {
    ...
    while (Date.now() - start < timeoutMs) {
        try {
            const ready = await page.evaluate(async () => {
                const vmokKey = Object.keys(window).find((k) => k.startsWith('__VMOK_@pc-im/im'));
                if (!vmokKey)
                    return { ok: false, reason: 'no-vmok' };
                const vmok = window[vmokKey];
                ...
                try {
                    ...
                    if (!svc)
                        return { ok: false, reason: 'no-imSdkService' };
                    return { ok: true };
                }
                catch (e) {
    ...
    throw new Error(`IM SDK 未就绪 (超时 ${timeoutMs}ms, lastErr=${lastErr ? String(lastErr) : 'n/a'})`);
}
