...
export class ImBridge {
     ...
     */
    async listConversations(waitMs = 15_000) {
        log.debug(`listConversations: 调用 SDK (waitMs=${waitMs})`);
        const items = await this.page.evaluate(async (waitMs) => {
            ...
            if (!cs.conversationMap || cs.conversationMap.size === 0)
                return [];
            ...
            for (const { cid, conv, secUid, uid } of convList) {
                ...
                items.push({
                    ...
                    lastMessageTs: ts,
                    unreadCount: typeof conv.unreadCount === 'number' ? conv.unreadCount : undefined,
                    isPinned: Boolean(conv.isPinned ?? conv.isStickOnTop ?? conv.pinTime),
                    isStranger: Boolean(conv.isStranger),
                    conversationType: typeof conv.conversationType === 'number' ? conv.conversationType : undefined,
                    conversationShortId: conv.conversationShortId !== undefined
     ...
     * 3. 通过 callback 收集结果
     */
    async sendTextMessage(uid, text) {
        ...
        const result = await this.page.evaluate(async (args) => {
            ...
            try {
                ...
                if (!conv) {
                    return { success: false, reason: 'no-conv', diag: [...diag] };
                }
                // 2. 创建 builder 并配置
                ...
                const result = await new Promise((resolve) => {
                    ...
                    try {
                        ...
                        if (sendPromise && typeof sendPromise.then === 'function') {
                            sendPromise.then((v) => { diag.push('sendMessage:resolved:' + (v === undefined ? 'undefined' : JSON.stringify(v).slice(0, 100))); }, (e) => {
                    ...
                    // 总体超时兜底
                    setTimeout(() => {
                        finish({ success: false, reason: 'timeout-15s', diag: [...diag] });
                    }, 15000);
                });
        ...
        log.info(`sendTextMessage: success=${result.success} msgId=${result.msgId} serverMsgId=${result.serverMsgId} reason=${result.reason || ''}`);
        if (result.diag)
            log.info(`sendTextMessage diag: ${result.diag.join(' | ')}`);
        if (result.reason && result.reason.startsWith('evaluate-error')) {
            log.error(`sendTextMessage evaluate 异常: ${result.reason}`);
            if (result.stack)
                log.error(`stack: ${result.stack}`);
        }
        return result;
    }
    /**
     ...
     */
    async getHistory(conversationId, opts = {}) {
        const waitMs = opts.waitMs ?? 5000;
        log.info(`getHistory: cid=${conversationId} waitMs=${waitMs}`);
        const items = await this.page.evaluate(async (args) => {
            const instance = await window.__getImInstance();
            const cs = instance.store.conversationStore;
     ...
     */
    async getHistoryByUid(uid, opts = {}) {
