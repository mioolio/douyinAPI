 ...
 */
export async function importFromCookieArray(cookies) {
    const cookiesMap = {};
 ...
 */
export async function loadFromStorageState(storageStatePath) {
    ...
    log.info(`从 storageState 加载 ${Object.keys(cookiesMap).length} 个 cookie（uid_tt=${cookiesMap['uid_tt'] || '?'}, sessionid=${cookiesMap['sessionid'] ? '有' : '无'}）`);
    return session;
}
// 重新导出路径常量方便外部使用
