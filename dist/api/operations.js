...
import { IMAPI_CONSTANTS, buildRequest, sendImapi, } from './imapi.js';
import { encodeVarintField, encodeStringField, encodeBytesField, parseFields, readString, readVarint, readVarintBigint, readVarintString, readMessage, findField, findFields, WireType, } from '../crypto/protobuf.js';
...
export function envFromSession(session) {
    return {
...
export async function listContacts(env) {
    ...
    try {
        while (pageCount < maxPages) {
            ...
            log.info(`listContacts: cmd=2006 page=${pageCount} cursor=${cursor} status=${resp.statusCode} desc=${resp.errorDesc} body=${resp.body.length}B`);
            ...
            log.info(`listContacts: page=${pageCount} got=${pageItems.length} hasMore=${hasMore} nextCursor=${nextCursor}`);
    ...
    try {
        ...
        log.info(`listContacts: cmd=1001 status=${resp.statusCode} desc=${resp.errorDesc} body=${resp.body.length}B`);
        if (resp.statusCode === 0) {
            const strangerItems = parseStrangerListResponse(resp, 1000);
            for (const it of strangerItems)
                it.isStranger = true;
 ...
 */
function parseUserConvListResponse(resp, bodyType) {
    const items = [];
    const bodyFields = parseFields(resp.body);
    ...
    const subFields = readMessage(subField);
    log.debug(`parseUserConvListResponse: sub-fields=${subFields.length} keys=${subFields.map((f) => f.field).join(',')}`);
    ...
    for (const f of findFields(subFields, 1)) {
        ...
        if (field6 && field6.wire === 2) {
            ...
            for (const pField of findFields(f6Fields, 1)) {
                ...
                if (pUid)
                    participants.push({ uid: pUid, secUid: pSecUid });
            }
        }
 ...
 */
function extractSecUid(field) {
    if (!field || field.wire !== WireType.LengthDelimited)
        return '';
    const buf = field.value;
    if (buf.length === 0)
        return '';
 ...
 */
export function detectMyUid(contacts) {
 ...
 */
function parseStrangerListResponse(resp, bodyType) {
    const items = [];
    const bodyFields = parseFields(resp.body);
    const subField = findField(bodyFields, bodyType);
    if (!subField) {
    ...
    const subFields = readMessage(subField);
    log.debug(`parseStrangerListResponse: sub-fields=${subFields.length} keys=${subFields.map((f) => f.field).join(',')}`);
...
export function extractUidFromCid(cid, myUid) {
...
export function buildPrivateCid(myUid, peerUid) {
 ...
 */
export async function getHistory(env, conversationId, opts) {
    ...
    const shortIdBig = typeof opts.conversationShortId === 'bigint'
    ...
    log.info(`getHistory: cid=${conversationId} shortId=${shortIdBig.toString()} cursor=${cursor} direction=${direction} limit=${limit}`);
    const resp = await sendImapi({
    ...
    if (resp.statusCode !== 0) {
        log.error(`getHistory: 失败 status=${resp.statusCode} desc=${resp.errorDesc} logId=${resp.logId}`);
        return [];
    }
 ...
 */
export async function getHistoryEx(env, conversationId, opts) {
    const limit = opts.limit ?? 30;
    ...
    const shortIdBig = typeof opts.conversationShortId === 'bigint'
        ? opts.conversationShortId
        : BigInt(opts.conversationShortId);
    const cursorBig = typeof cursorIn === 'bigint' ? cursorIn : BigInt(cursorIn);
    const bodyType = 301;
    const subBody = Buffer.concat([
        encodeStringField(1, conversationId),
        encodeVarintField(2, 1), // conversation_type=1 私聊
    ...
    log.info(`getHistoryEx: cid=${conversationId} shortId=${shortIdBig.toString()} cursor=${cursorBig.toString()} direction=${direction} limit=${limit}`);
    const resp = await sendImapi({
        path: '/v1/message/get_by_conversation',
        body: reqBuf,
        cookie: env.cookie,
    ...
    if (resp.statusCode !== 0) {
        log.error(`getHistoryEx: 失败 status=${resp.statusCode} desc=${resp.errorDesc} logId=${resp.logId}`);
 ...
 */
export async function getHistoryAll(env, conversationId, opts) {
    ...
    while (hasMore && pageCount < maxPages && allMessages.length < maxMessages) {
        ...
        log.info(`getHistoryAll: page=${pageCount} got=${result.messages.length} new=${newCount} total=${allMessages.length} hasMore=${result.hasMore} nextCursor=${result.nextCursor.toString()}`);
        if (opts.onProgress) {
    ...
    log.info(`getHistoryAll: 共拉取 ${pageCount} 页，${allMessages.length} 条消息（去重后）`);
    return allMessages;
}
/** 解析 cmd=301 响应 body 为消息列表（含分页元数据） */
function parseHistoryResponseEx(resp, bodyType, expectedCid, myUid) {
    const items = [];
    const bodyFields = parseFields(resp.body);
    const subField = findField(bodyFields, bodyType);
    if (!subField) {
    ...
    const subFields = readMessage(subField);
    log.debug(`parseHistoryResponseEx: sub-fields=${subFields.length} keys=${subFields.map((f) => f.field).join(',')}`);
    // field 1 (repeated MessageBody): messages
    ...
    const serverNextCursor = cursorField && cursorField.wire === WireType.Varint
        ? cursorField.value
        : 0n;
    // field 3 (bool): has_more
...
function parseHistoryResponse(resp, bodyType, expectedCid, myUid) {
    return parseHistoryResponseEx(resp, bodyType, expectedCid, myUid).messages;
}
/** 解析 MessageBody 为消息条目 */
function parseMessageBody(f, expectedCid, myUid) {
    try {
        ...
        const isFromRobot = ext['a:is_from_robot'] === '1' ||
            aweType === 716;
        // 识别消息类别
        ...
        if (contentObj && typeof contentObj === 'object') {
            ...
            if (aweType === 276 || typeof contentObj.tips === 'string') {
                category = 'system_tip';
            ...
            // 视频分享（aweType=800, msgType=8）
                ...
                // 如果有描述
                if (!text && contentObj.desc)
                    text = String(contentObj.desc);
            }
 ...
 */
export async function getConversationInfo(env, conversationIds) {
    const bodyType = 610;
    const entries = conversationIds.map((cid) => {
    ...
    if (resp.statusCode !== 0) {
        log.error(`getConversationInfo: 失败 status=${resp.statusCode} desc=${resp.errorDesc} logId=${resp.logId}`);
        return [];
    }
    return parseInfoResponse(resp, bodyType);
}
/** 解析 cmd=610 响应 */
function parseInfoResponse(resp, bodyType) {
    const items = [];
    const bodyFields = parseFields(resp.body);
    const subField = findField(bodyFields, bodyType);
    if (!subField)
 ...
 */
export async function sendMessage(env, conversationId, text, sign) {
    const clientMessageId = randomUUID();
    const now = Date.now();
    // conversation_short_id 是 int64，可能超过 2^53（JS number 精度上限）
    // 必须用 BigInt 编码，否则会精度丢失导致消息发送失败（status=3）
    const shortIdBig = typeof sign.conversationShortId === 'bigint'
    ...
    const query = {};
    if (sign.aBogus)
        query.a_bogus = sign.aBogus;
    if (sign.msToken)
        query.msToken = sign.msToken;
    ...
    log.info(`sendMessage: cid=${conversationId} shortId=${shortIdBig} text=${JSON.stringify(text)} clientMsgId=${clientMessageId} signed=${Boolean(sign.aBogus && sign.msToken && sign.bdTicketGuardClientData)}`);
    try {
        const resp = await sendImapi({
        ...
        if (status === 0 && serverId > 0) {
            log.info(`sendMessage: 成功 serverMsgId=${serverId} logId=${resp.logId}`);
            return {
                success: true,
                msgId: respClientId,
 ...
 */
export async function sendImage(env, conversationId, image, sign) {
    const clientMessageId = randomUUID();
    const now = Date.now();
    const shortIdBig = typeof sign.conversationShortId === 'bigint'
        ? sign.conversationShortId
    ...
    extraHeaders['bd-ticket-guard-web-sign-type'] = sign.bdTicketGuardWebSignType || '1';
    extraHeaders['bd-ticket-guard-web-version'] = sign.bdTicketGuardWebVersion || '2';
    log.info(`sendImage: cid=${conversationId} shortId=${shortIdBig} oid=${image.oid} ` +
 ...
 */
export async function getIdentitySecurityToken(env, sign) {
    ...
    if (sign.msToken)
        url.searchParams.set('msToken', sign.msToken);
    if (sign.aBogus)
        url.searchParams.set('a_bogus', sign.aBogus);
    const headers = {
